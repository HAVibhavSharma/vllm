...
class AgentPrefetchRequest(BaseModel):
    ...
    prefill_on_miss: bool = Field(
        ...
        "Default True, which is safe only because the scheduler will not "
        "admit such a phantom into a step that has real work running: it is "
        "held in the skipped-waiting queue until the batch is idle (see "
        "`VLLM_PREFETCH_PREFILL_MAX_RUNNING`) and dropped without prefilling "
        "if no idle step arrives before "
        "`VLLM_PREFETCH_PREFILL_DEFER_TIMEOUT_S`. So a mispredicted warm "
        "spends idle GPU time, never a real request's token budget. Set "
        ...
        "Costs one prefill per warmed prefix. The deferral keeps that cost "
        "off other requests' token budget, but it is still GPU time: a wrong "
        "guess spends an idle step that a later real request could have had "
        "to itself, and it materialises blocks that push others out of "
        "HBM.\n\n"
