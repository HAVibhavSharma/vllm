...
class AgentPrefetchRequest(BaseModel):
    ...
    prefill_on_miss: bool = Field(
        ...
        "Default True, and by default unthrottled: the phantom prefills in "
        "the first step it reaches, taking token budget from whatever real "
        "work is in that step. A mispredicted warm therefore costs another "
        "request's TTFT, not just idle GPU time, and the prefetch hit rate "
        "will not show it -- the phantom lands either way, so the number "
        "that moves is the TTFT of the cold request that was prefilling "
        "alongside it.\n\n"
        "Set `VLLM_PREFETCH_PREFILL_MAX_RUNNING` to a non-negative value to "
        "throttle it: the phantom is then admitted only into a step running "
        "at most that many real requests, held in the skipped-waiting queue "
        "otherwise, and dropped without prefilling if no such step arrives "
        "before `VLLM_PREFETCH_PREFILL_DEFER_TIMEOUT_S` (which is inert "
        "while uncapped). 1 confines it to decode-only batches, where the "
        "budget is nearly all spare. 0 is stricter still and usually "
        "self-defeating -- it waits for a fully idle step, which in an agent "
        "workflow does not arrive until the previous request has finished, "
        "by which point the next one is already queued. Set "
        ...
        "or not the guess was right: uncapped it comes out of the token "
        "budget of whatever shares its step, at 1 it lengthens a step a "
