...
class AgentPrefetchRequest(BaseModel):
    ...
    prefill_on_miss: bool = Field(
        ...
        "`VLLM_PREFETCH_PREFILL_MAX_RUNNING` real requests (1 by default, so "
        "it overlaps a decode-only batch but not a prefill-heavy one), held "
        "in the skipped-waiting queue otherwise, and dropped without "
        "prefilling if no such step arrives before "
        "`VLLM_PREFETCH_PREFILL_DEFER_TIMEOUT_S`. So a mispredicted warm "
        "costs a decoding request one longer step, never a competing "
        "prefill's token budget. Set that variable to 0 for the stricter "
