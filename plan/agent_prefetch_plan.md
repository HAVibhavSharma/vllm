...
> | LMCache miss on a phantom | Not considered — the phantom simply falls through to normal admission. | **Changed 2026-08-10.** `Scheduler.schedule()` aborts a `prefetch_only` request when the connector reports `ext_tokens == 0`, and `_finish_prefetch_only_misses()` finishes it as `FINISHED_STOPPED` / `FinishReason.STOP` without ever scheduling prefill. Previously it ran a full prefill and STOREd to LMCache — ~`delta_cold` spent on a prefix nobody had requested, serialized ahead of real traffic at low `--max-num-seqs`. Counted as `_prefetch_only_misses_total`; `LMCACHE_MP_FULL_HIT_ONLY=1` extends the abort to partial hits. See `plan/architecture.md` §6 and `plan/new-eviction/12-first-run-findings.md` §7. |
>
