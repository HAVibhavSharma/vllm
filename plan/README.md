...
| [`hbm-logging-changes.md`](hbm-logging-changes.md) | The `kv_hbm` log line — HBM block accounting, prefix-cache hit rate, KV **rematerialisation** (the movement number), and `job:node` eviction attribution. Added to both this branch and the LRU baseline clone for comparison. | **As-built** (2026-08-07); tests written, not yet run. |
